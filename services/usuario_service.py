from database.connection import connect, is_unique_constraint_error
...
def criar_usuario_padrao():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        """)
        conn.commit()
        print("Usuario padrao criado: login 'admin', senha 'admin'")
    conn.close()
...
def registrar_usuario(nome, login, senha, nivel="usuario"):
    ...
    try:
        ...
        if is_unique_constraint_error(exc):
            mensagem = "Esse nome de usuario (login) ja esta em uso. Escolha outro."
        else:
            mensagem = f"Erro ao criar usuario: {exc}"
    finally:
        conn.close()
...
def atualizar_dados_usuario(usuario_id, nome, login, foto=None):
    ...
    try:
        cursor.execute(
            "UPDATE usuarios SET nome = ?, login = ?, foto = ? WHERE id_usuario = ?",
            (nome, login, foto, usuario_id),
        )
        conn.commit()
        sucesso = True
        ...
        sucesso = False
        if is_unique_constraint_error(exc):
            mensagem = "Esse login ja esta em uso."
        else:
            mensagem = f"Erro ao atualizar os dados: {exc}"
    finally:
        conn.close()
